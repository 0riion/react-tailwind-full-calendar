...
type TUnit = "year" | "month" | "hour" | "minute" | "second";

interface AdjustDateTimeAction {
...
const reducer = (state: CalendarState, action: any): CalendarState => {
  switch (action.type) {
    ...
    case ActionTypes.AdjustDateTime:
      ...
      const newDate = AdjustDateTimeAction.increment
        ? state.selectedDate.add(1, AdjustDateTimeAction.unit)
        : state.selectedDate.subtract(1, AdjustDateTimeAction.unit);
